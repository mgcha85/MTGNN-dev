...
def plot_forecast(data,forecast,confidence,ss,index,col):
    ...
    colours = [
    "Black", "Red", "Green", "Blue", "Yellow", "Cyan", "Magenta", "Orange", "Purple", "Brown",
    "Maroon", "Navy", "Olive", "Teal", "Lime", "Aqua", "Fuchsia", "Indigo", "Crimson", "Sienna",
    "Coral", "Orchid", "Tomato", "Salmon", "Gold", "Chocolate", "Plum", "Turquoise",
    "Lavender", "MidnightBlue", "DarkSlateBlue", "DarkCyan", "DarkViolet", "DarkOrange",
    "DarkRed", "DarkOliveGreen", "DarkMagenta", "DarkGoldenrod", "DarkBlue", "DarkTurquoise",
    "DarkGreen", "DarkSlateGray", "DarkKhaki", "DarkSalmon", "DarkOrchid", "DarkGray", "DarkSeaGreen",
    "DarkSlateGrey", "FireBrick", "ForestGreen", "HotPink", "IndianRed", "LightCoral", "LightSalmon",
    "MediumAquamarine", "MediumBlue", "MediumOrchid", "MediumPurple", "MediumSeaGreen", "MediumSlateBlue",
    "MediumSpringGreen", "MediumTurquoise", "MediumVioletRed", "PaleGreen", "PaleTurquoise",
    "PaleVioletRed", "PapayaWhip", "RosyBrown", "RoyalBlue", "SaddleBrown", "SandyBrown", "SeaGreen",
    "Silver", "SkyBlue", "SlateBlue", "SpringGreen", "SteelBlue", "Tan", "Thistle", "Tomato",
    "Turquoise", "Violet", "Wheat", "YellowGreen", "DarkOliveGreen", "DarkSeaGreen", "LightSeaGreen",
    "MediumSeaGreen", "OliveDrab", "PaleGreen", "SeaGreen", "SpringGreen", "DarkGreen", "Green",'grey',
    "LimeGreen", "YellowGreen", "SlateGray"  # Adding "SlateGray"
    ]

    c_plot=0
    ss_o=ss[:]
    for start in range(0,98,10):
        ss=ss_o[start:min(start+10,len(ss_o))]
        print(start,'-',min(start+10,len(ss_o)),',',len(ss))

        pyplot.style.use("seaborn-dark") 
        fig = pyplot.figure()
        ax = fig.add_axes([0.1, 0.1, 0.7, 0.7])

        for counter,s in enumerate(ss):
            #Plot the forecast of the solution
        ...
        pyplot.title('PT Cycle', y=1.03,fontsize=18)
        ...
        pyplot.savefig(images_dir+'PT Cycle Zero One Scale'.replace('/','_')+str(c_plot)+'.png', bbox_inches="tight")
        pyplot.savefig(images_dir+'PT Cycle Zero One Scale'.replace('/','_')+str(c_plot)+".pdf", bbox_inches = "tight", format='pdf')
...
for i in range(all.shape[0]):
    for j in range(all.shape[1]):
            ...
            all_n[i,j]=all[i,j]/torch.max(all[:,j])#added for visibility of scale [scale between 0 and 1]
...
ss=[]
#plot all forecasted solutions
for attack, solutions in graph.items():
    for s in solutions:
        if not s in ss:
            ss.append(s)
plot_forecast(smoothed_dat[:-36,],smoothed_dat[-36:,], smoothed_confidence,ss,index,col)
