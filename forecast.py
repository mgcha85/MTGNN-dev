...
def map_name_to_abbreviation(name):
    ...
    with open('abb.txt', 'r') as file:
        for line in file:
            abbreviation, full_name = map(str.strip, line.split(': '))
...
for i,c in enumerate(col):
    col[i]=consistent_name(col[i])
    col[i]=map_name_to_abbreviation(col[i])
...
names=[consistent_name(c) for c in col]
mentions_names=[consistent_name(c) for c in col]
for i,m in enumerate(mentions_names):
    if i in range(16,42):
        mentions_names[i]+=' (M)'

model.visualize_attention_scores(names,all_attacks_r,tech_r,'Attacks_PATs')
model.visualize_attention_scores(mentions_names,attacks_r,attacks_mentions_r,'Attacks_Mentions')
...
#2
#let's now visualise some input and output of relevant nodes?
a_i=index['Mentions-Adversarial Attack']
b_i=index['Solution_ADVERSARIAL TRAINING_Mentions']
a_x_y=torch.cat((X[0,0,a_i,:],(Y/scale)[:,a_i]),dim=0)
b_x_y=torch.cat((X[0,0,b_i,:],(Y/scale)[:,b_i]),dim=0)
model.visualise_data(a_x_y,b_x_y,'Adversarial Attack','Adversarial Training')

col=o_col[:]


#3
#saliency maps for timestep importance
time_saliency_maps=[]
saliency_map_36=None
node=index['Malware-ALL']
for t in range(0,36):
