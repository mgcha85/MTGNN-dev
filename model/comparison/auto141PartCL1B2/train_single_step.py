...
def plot_data(data,title):
    ...
    fig = plt.gcf()
    #plt.savefig('model/comparison/auto141PartCL1B2/'+title[0:4]+str(counter+1)+'.png', bbox_inches="tight")
...
###########################################################################################################
    ...
    with open('model/comparison/auto141PartCL1B2/'+type+'/'+title+'_'+type+'.txt',"w") as f:
...
def plot_predicted_actual(predicted, actual, title, type,counter):
    ...
    fig = plt.gcf()
    title=title.replace('/','_')
    plt.savefig('model/comparison/auto141PartCL1B2/'+type+'/'+title+'_'+type+'.png', bbox_inches="tight")
    #plt.savefig('model/comparison/auto141PartCL1B2/'+type+'/'+title+'_'+type+".pdf", bbox_inches = "tight", format='pdf')
...
def evaluate_sliding_window(data, test_window, model, evaluateL2, evaluateL1, n_input, is_plot):
    ...
    for i in range(n_input, test_window.shape[0],data.out_len): # X.shape[0]=n_input+36 (as example)?
        ...
        y_true = test_window[i: i+data.out_len,:].clone() #10x141
...
###########################################################################################################
    ...
    if is_plot:
        for v in range(r,r+1):
            ...
            #plot
            plot_predicted_actual(predict[:,col],Ytest[:,col],node_name, 'Testing', counter)
            counter+=1
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, is_plot):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        scale = data.scale.expand(Y.size(0), Y.size(1), data.m) #scale will have the max of each column (141 max values)
...
def train(data, X, Y, model, criterion, optim, batch_size, split):
    model.train()
    total_loss = 0
    n_samples = 0
    iter = 0
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        num_sub = int(args.num_nodes / split)

        if iter%args.step_size==0:
            T+=6 
        T=min(T, args.seq_out_len)
        ...
        for j in range(split):
            ...
            loss = criterion(output[:,:T,:], ty[:,:T,:])
            loss.backward()
            total_loss += loss.item()
            n_samples += (output.size(0) * T * len(id))
        ...
        # if iter%1==0:
        #     print('iter:{:3d} | loss: {:.3f}'.format(iter,loss.item()/(output.size(0) * T * data.m)))
        iter += 1
...
parser.add_argument('--save', type=str, default='model/comparison/auto141PartCL1B2/model.pt',
...
def main(experiment,acc):
    ...
    for q in range(20):
        ...
        split=splits[randrange(len(splits))] 
        k=min(ks[randrange(len(ks))], int(args.num_nodes/split)-1)# if size of topk elements is larger than subgraph size-1, k=subgraph size-1
        ...
        try:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                print('Iter:',q)
                print('epoch:',epoch)
                print('hp=',[gcn_depth,lr,conv,res,skip,end, split,k, dropout, dilation_ex, node_dim, prop_alpha, tanh_alpha, layer, epoch])
                ...
                train_loss = train(Data, Data.train[0], Data.train[1], model, criterion, optim, args.batch_size,split)
                ...
                if (not math.isnan(val_corr)) and val_loss < best_rse:
                    ...
                    with open('model/comparison/auto141PartCL1B2/model'+str(experiment)+'.pt', 'wb') as f:
                        torch.save(model, f)
                    best_val = sum_loss
                    best_rse= val_loss
                    best_rae= val_rae
                    ...
                    best_hp=[gcn_depth,lr,conv,res,skip,end, split, k, dropout, dilation_ex, node_dim, prop_alpha, tanh_alpha, layer, epoch]
    ...
    with open('model/comparison/auto141PartCL1B2/model'+str(experiment)+'.pt', 'rb') as f:
        model = torch.load(f)
...
if __name__ == "__main__":
    ...
    smape=[]
    for i in range(10):
        val_acc, val_rae, val_corr, val_smape, test_acc, test_rae, test_corr, test_smape = main(i,acc)
        vacc.append(val_acc)
        vrae.append(val_rae)
