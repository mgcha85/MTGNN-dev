...
def plot_data(data,title):
    ...
    fig = plt.gcf()
    #plt.savefig('model/comparison/predefined123Uni/'+title[0:4]+str(counter+1)+'.png', bbox_inches="tight")
...
def save_metrics_1d(predict, test, index):
...
###########################################################################################################
    ...
    file_name=DataLoaderS.col[index].replace('-ALL','').replace('Mentions-','Mentions of ').replace(' ALL','').replace('Solution_','').replace('_Mentions','').replace('/',' - ')
    file_name=consistent_name(file_name)
    with open('model/comparison/predefined123Uni/'+file_name+'.txt',"w") as f:
...
def plot_predicted_actual(predicted, actual, title, type,counter):
    ...
    plt.savefig('model/comparison/predefined123Uni/'+title+'_'+type+'.png', bbox_inches="tight")
...
def evaluate_sliding_window(data, test_window, model, evaluateL2, evaluateL1, n_input, is_plot):
    ...
    scale = data.scale.expand(test_window.size(0), data.m) #scale will have the max of each column (123 max values)
    ...
    for i in range(n_input, test_window.shape[0],data.out_len): # X.shape[0]=n_input+36 (as example)?
        ...
        print('**************x_input*******************')
        # scale = data.scale.expand(x_input.size(0), data.m) #scale will have the max of each column (123 max values)
        ...
        with torch.no_grad():
            output = model(X)  
        y_pred = output[-1, :, :,-1].clone()#10x123
        y_true = test_window[i: i+data.out_len,:].clone() #10x123
        ...
        # scale = data.scale.expand(y_pred.size(0), data.m) #scale will have the max of each column (123 max values)
    ...
    scale = data.scale.expand(test.size(0), data.m) #scale will have the max of each column (123 max values)
...
###########################################################################################################
    ...
    if is_plot:
        for v in range(r,r+1):
            ...
            #save error to file
            save_metrics_1d(torch.from_numpy(predict[:,col]),torch.from_numpy(Ytest[:,col]),col)
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, is_plot):
    ...
    sum_squared_diff=0
    sum_absolute_diff=0
    r=0 #random.randint(0, 122)#random column
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        scale = data.scale.expand(Y.size(0), Y.size(1), data.m) #scale will have the max of each column (123 max values)
...
##########################################################################################################
    ...
    # scale = data.scale.expand(test.size(0), test.size(1), data.m) #scale will have the max of each column (123 max values)
    # test_s= test*scale
    test_s=test
...
###########################################################################################################
    ...
    if is_plot:
        for v in range(r,r+1):
            ...
            save_metrics_1d(torch.from_numpy(predict[-1,:,col]),torch.from_numpy(Ytest[-1,:,col]),col)

            #plot_predicted_actual(predict[0,:,col],Ytest[0,:,col],'Validation of node '+str(col)+', RRSE= '+str(round(rrse,2))+', CORR= '+str(round(correlation,2)),counter)
            node_name=DataLoaderS.col[col].replace('-ALL','').replace('Mentions-','Mentions of ').replace(' ALL','').replace('Solution_','').replace('_Mentions','')
            node_name=consistent_name(node_name)
...
parser.add_argument('--data', type=str, default='./data/sm_data_g.txt',
                    help='location of the data file')
parser.add_argument('--log_interval', type=int, default=2000, metavar='N',
                    help='report interval')
parser.add_argument('--save', type=str, default='model/comparison/predefined123Uni/model.pt',
...
parser.add_argument('--num_nodes',type=int,default=123,help='number of nodes/variables')
