...
def plot_data(data,title):
    ...
    fig = plt.gcf()
    #plt.savefig('model/comparison/auto141PartCL3/'+title[0:4]+str(counter+1)+'.png', bbox_inches="tight")
...
###########################################################################################################
    ...
    with open('model/comparison/auto141PartCL3/'+type+'/'+title+'_'+type+'.txt',"w") as f:
...
def plot_predicted_actual(predicted, actual, title, type,counter):
    ...
    fig = plt.gcf()
    title=title.replace('/','_')
    plt.savefig('model/comparison/auto141PartCL3/'+type+'/'+title+'_'+type+'.png', bbox_inches="tight")
    #plt.savefig('model/comparison/auto141PartCL3/'+type+'/'+title+'_'+type+".pdf", bbox_inches = "tight", format='pdf')
...
def train(data, X, Y, model, criterion, optim, batch_size, split, T):
    model.train()
    total_loss = 0
    n_samples = 0
    iter = 0
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        for j in range(split):
            ...
            n_samples += (output.size(0) * T * data.m)
...
parser.add_argument('--save', type=str, default='model/comparison/auto141PartCL3/model.pt',
    ...
    for q in range(60):
        ...
        try:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                train_loss = train(Data, Data.train[0], Data.train[1], model, criterion, optim, args.batch_size,split, T)
